                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            file_id = mongodb_file_manager.upload_to_mongodb(pdf_bytes_data, pdf_metadata)
                            logger.info(f"[PARALLEL] {vendor_name}: Saved PDF to MongoDB (ID: {file_id})")
                        except Exception as e:
            ...
            for result in split_results:
                ...
                for model in result.get("models", []):
                    ...
                    try:
                        ...
                        mongodb_file_manager.upload_json_data(single_family_payload, metadata)
                        logger.info(f"[PARALLEL] {vendor_name}: Saved to MongoDB: {safe_vendor}/{safe_ptype}/{safe_series}")
                    except Exception as e:
...
def main():
    ...
    # Check which product types already have JSON files in specs folder
    folder_path = "./specs/"
    file_pattern = "*.json"

    # Get a list of existing product_type files
    existing_files = glob.glob(os.path.join(folder_path, file_pattern))
    existing_product_types = [os.path.splitext(os.path.basename(file))[0] for file in existing_files]
