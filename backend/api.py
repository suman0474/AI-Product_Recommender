...
def agentic_validate():
    ...
    try:
        ...
        try:
            ...
            # Build a normalized lookup dict for template descriptions
            # Maps normalized_key -> description
            normalized_template_lookup = {}
            for template_key, description in field_descriptions.items():
                normalized_template_lookup[normalize_key(template_key)] = description
            ...
            for field_key in all_keys:
                ...
                # Try to find a matching template description using normalized key
                if normalized_name in normalized_template_lookup:
                    # Found match via normalized key
                    field_descriptions[field_key] = normalized_template_lookup[normalized_name]
                    field_descriptions[field_name] = normalized_template_lookup[normalized_name]
                elif field_name not in field_descriptions and field_key not in field_descriptions:
                    # No match found, generate a readable description from the field name
