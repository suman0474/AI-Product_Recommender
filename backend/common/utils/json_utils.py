...
def extract_json_from_response(text: str) -> Optional[Dict[str, Any]]:
    ...
    # Try to find JSON object in text by locating outermost braces
    try:
        start = text.find('{')
        end = text.rfind('}')
        if start != -1 and end != -1 and end > start:
            ...
            try:
                ...
                # Try sanitizing if direct load fails
                sanitized = sanitize_json_string(json_candidate)
    ...
    for i, char in enumerate(text):
        if char == '"' and not escape_next:
            ...
            if char == '\n':
                ...
                # Other control chars - skip or escape? Skip for safety
                pass
            else:
                result.append(char)
        else:
            # Outside string: skip whitespace/control chars, keep structure
            if char in ' \t\n\r':
                continue
            result.append(char)
