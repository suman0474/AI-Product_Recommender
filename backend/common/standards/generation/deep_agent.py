...
# FIX: Increased per-domain limit - ALL specs should be included
# User requirement: Include ALL technical specifications regardless of count
MAX_SPECS_PER_DOMAIN = 500  # Increased to allow comprehensive domain specs
...
# default_section captures content before first section marker as "DEEP_AGENT"

PLANNER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["PLANNER"]
WORKER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["WORKER"]
SYNTHESIZER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["SYNTHESIZER"]
MERGER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["MERGER"]
...
ITERATIVE_WORKER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["ITERATIVE_WORKER"]
...
def load_standard_text(standard_type: str) -> Optional[str]:
    ...
    # Cache miss - load from Blob Storage
    ...
    try:
        ...
        # Load using AzureBlobFileManager
        logger.debug(f"Loading standard document from Blob: {filename}")
        
        # Import dynamically to avoid circular dependencies if any, though top-level is fine usually
        # But we'll put it top-level in next step if needed. For now local import is safe.
        ...
        if not blob_manager.file_exists(blob_path, container_name):
            ...
            if matching_blobs:
                ...
                blob_path = candidate
                 ...
                 logger.warning(
                     f"Standard file not found in Blob Storage (checked suffix '{filename}'): {container_name}. "
                     f"Upload '{filename}' to container '{container_name}' to enable {standard_type} domain analysis."
                 )
                 return None
    ...
    for standard_type in STANDARD_FILES.keys():
        try:
            ...
            if result:
                ...
                logger.info(
                    f"[DOC_CACHE] SKIPPED: {standard_type} - document not found in Blob Storage "
                    f"(app can still function without it)"
                )
        except Exception as e:
            failed.append(standard_type)
...
def run_standards_deep_agent(
    ...
    try:
        ...
        if target_reached:
            logger.info(f"[DEEP_AGENT] SUCCESS - Target reached: {final_count} specs (minimum: {min_required}) in {iteration} iterations")
        else:
            logger.warning(f"[DEEP_AGENT] FAILED - Target NOT reached: {final_count} specs (minimum: {min_required}) after {iteration} iterations")
...
BATCH_PLANNER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["BATCH_PLANNER"]
BATCH_WORKER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["BATCH_WORKER"]
BATCH_SYNTHESIZER_PROMPT = STANDARDS_DEEP_AGENT_PROMPTS["BATCH_SYNTHESIZER"]
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            if item.get('sample_input'):
                # FIX: Removed [:100] truncation - planner needs full context with ALL specs
                item_desc += f" - {item.get('sample_input', '')}"
            ...
            for i, item in enumerate(items):
                ...
                if should_include:
                    relevant_items.append({
                        ...
                        "sample_input": item.get('sample_input', '')  # FIX: Removed [:200] truncation to include ALL specs
                    })
                ...
                for domain in domains_to_try:
                    ...
                    for item_info in items_needing_more[:]:  # Copy list for safe removal
                        ...
                        if item_info["current_count"] >= MAX_SPECS_PER_ITEM:
                            ...
                            logger.info(f"[BATCH] SUCCESS - Item '{item_info['name']}' reached max cap ({item_info['current_count']}/{MAX_SPECS_PER_ITEM} specs)")
                        elif item_info["current_count"] >= MIN_STANDARDS_SPECS_COUNT:
                            items_needing_more.remove(item_info)
                            logger.info(f"[BATCH] SUCCESS - Item '{item_info['name']}' reached minimum ({item_info['current_count']} specs)")
