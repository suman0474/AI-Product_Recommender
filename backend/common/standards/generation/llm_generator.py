...
LLM_SPECS_MODEL = AgenticConfig.FLASH_MODEL

# Model for deep reasoning / key discovery (uses centralized config)
REASONING_MODEL = AgenticConfig.FLASH_MODEL
    ...
    if _specs_llm is None:
        with _llm_lock:
            if _specs_llm is None:
                ...
                from common.agentic.context_managers import GlobalResourceRegistry, ResourceMetrics, ResourceStatus
...
# Minimum number of specifications that LLM must generate (30 from LLM + 30 from standards = 60 total)
...
KEY_DISCOVERY_PROMPT = """You are an industrial instrumentation expert. Analyze the product type and discover all relevant specification keys.

PRODUCT TYPE: {product_type}
CATEGORY: {category}
CONTEXT: {context}

TASK: Identify ALL relevant specification keys for this product type, organized by importance.

OUTPUT (JSON only):
{{
  "mandatory_keys": ["accuracy", "measurement_range", "output_signal", "process_connection", "supply_voltage"],
  "optional_keys": ["ambient_temperature", "protection_rating", "mounting_type", "display_type"],
  "safety_keys": ["sil_rating", "atex_zone", "hazardous_area_classification"],
  "all_keys": ["<combined list of all keys>"],
  "product_analysis": "<brief analysis of this product type and its key characteristics>"
}}"""
...
LLM_SPECS_GENERATION_PROMPT = """You are an industrial instrumentation expert. Generate comprehensive technical specifications for the given product type.
...
LLM_SPECS_ITERATIVE_PROMPT = """You are an industrial instrumentation expert. Generate ADDITIONAL specifications not already listed.

PRODUCT TYPE: {product_type}
CATEGORY: {category}
CONTEXT: {context}

EXISTING SPECIFICATIONS (DO NOT DUPLICATE):
{existing_specs}

TASK: Generate {specs_needed} NEW specifications not in the existing list above.

RULES:
- NO duplicates with existing specifications
- Clean technical values only
- Focus on less common but important specs

OUTPUT (JSON only):
{{
  "specifications": {{
    "<new_spec_key>": {{"value": "<value>", "confidence": 0.8}}
  }}
}}"""
...
def _flatten_and_clean_specs(raw_specs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ROOT CAUSE FIX: Now filters out error messages and invalid values.

    Args:
        raw_specs: Raw specifications from LLM response

    Returns:
        Cleaned and flattened specifications dict (with errors filtered out)
    """
    ...
    ERROR_PATTERNS = [
        ...
        "unable to", "cannot ", "connection error"
    ]

    def is_error_value(val: str) -> bool:
        """Check if a value looks like an error message."""
        if not val:
            return False
        val_lower = val.lower()
        ...
        if any(pattern in val_lower for pattern in ERROR_PATTERNS):
            return True
        # Check for overly long values (likely descriptions/errors)
        if len(val) > 150:
            return True
        return False
    ...
    for key, value in raw_specs.items():
        ...
        if key.lower() in ['error', 'error_message', 'exception']:
            logger.debug(f"[_flatten_and_clean_specs] Skipping error key: {key}")
            continue

        if isinstance(value, dict):
            # ROOT CAUSE FIX: Skip error dicts
            if 'error' in value and 'value' not in value:
                logger.debug(f"[_flatten_and_clean_specs] Skipping error dict for key: {key}")
                continue

            raw_value = value.get("value", str(value))

            # ROOT CAUSE FIX: Skip error message values
            if is_error_value(str(raw_value)):
                logger.debug(f"[_flatten_and_clean_specs] Filtered error value for '{key}': {str(raw_value)[:50]}...")
                continue
            ...
            # ROOT CAUSE FIX: Skip error message values
            if is_error_value(str(value)):
                logger.debug(f"[_flatten_and_clean_specs] Filtered error value for '{key}': {str(value)[:50]}...")
                continue
...
USER_SPECS_EXTRACTION_PROMPT = """You are a specification extractor. Extract ONLY explicitly mentioned specifications from the user input. NO inference, NO assumptions.
