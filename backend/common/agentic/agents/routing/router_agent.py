...
Usage:
    from common.agentic.agents.routing.router_agent import RouterAgent
...
try:
    from common.observability.structured_logger import (
        StructuredLogger,
...
try:
    from common.concurrency.distributed_lock import create_distributed_lock
    from common.concurrency.optimistic_lock import ConcurrentModificationError
...
try:
    from .semantic_classifier import WorkflowType
    logger.debug("[RouterAgent] Using canonical WorkflowType from semantic_classifier")
except ImportError:
    # Fallback: Define locally if semantic_classifier unavailable
    class WorkflowType(Enum):
        """Available workflow types for routing (fallback definition)."""
        SOLUTION = "solution"
        PRODUCT_SEARCH = "product_search"
        INSTRUMENT_IDENTIFIER = "instrument_identifier"
        ENGENIE_CHAT = "engenie_chat"
        OUT_OF_DOMAIN = "out_of_domain"
    logger.warning("[RouterAgent] Using local WorkflowType (semantic_classifier unavailable)")
...
class RouterAgent:
    ...
    def __init__(
        ...
        if distributed_lock:
            ...
            from common.concurrency.distributed_lock import InMemoryDistributedLock
