...
# ROOT CAUSE FIX: Semantic field aliases for common UI display names
# Maps common schema field names to their equivalent names in defaults
FIELD_NAME_ALIASES = {
    # SIL/Safety
    "sil rating": ["safety integrity level", "sil capability", "sil", "functional safety"],
    "safety integrity level": ["sil rating", "sil capability", "sil"],
    # Temperature
    "ambient temperature": ["ambient temperature range", "operating temperature", "operating ambient"],
    ...
    "humidity": ["humidity range", "relative humidity"],
    # Materials
    "wetted material": ["wetted parts", "process wetted material"],
    "wetted parts": ["wetted material", "process wetted material"],
    ...
    # Certifications
    "certifications": ["certification", "approvals", "product certifications"],
    "ce marking": ["ce mark", "ce certified"],
    "explosion proof": ["explosion protection", "hazardous area"],
    # Electrical
...
def _get_field_aliases(field_name_normalized: str) -> List[str]:
    """
    ROOT CAUSE FIX: Get aliases for a field name to try matching.

    Args:
        field_name_normalized: Normalized field name (lowercase with spaces)

    Returns:
        List of alias names to try for matching
    """
    ...
    # Also check partial matches (field_name is part of an alias key)
...
def get_default_value_for_field(product_type: str, field_name: str) -> Optional[str]:
    ...
    # ROOT CAUSE FIX: Get aliases to try for this field
    ...
    # Lowered threshold from 0.5 to 0.2 for better matching (ROOT CAUSE FIX)
    if best_score >= 0.2:
