...
from common.infrastructure.state.execution.executor_manager import get_global_executor
...
from common.standards.generation.normalizer import normalize_specification_output, normalize_key
from common.standards.generation import run_standards_deep_agent_batch
from common.standards.shared.constants import MIN_TOTAL_SPECS_COUNT
from .normalizers import is_valid_spec_value, is_valid_spec_key
# SpecificationAggregator imported lazily inside functions to avoid circular import
...
from common.infrastructure.state.context.langchain import LangChainLLMClientManager
from common.infrastructure.state.context.managers import GlobalResourceRegistry
from common.config.agentic_config import AgenticConfig

_llm_manager: Optional[LangChainLLMClientManager] = None
_llm_lock = threading.Lock()


def get_shared_llm(temperature: float = 0.1, model: str = None):
    """
    ...
    Uses gemini-2.5-flash by default for faster spec generation with fewer rate limits.
    ...
    # Use FLASH_MODEL for spec generation (fast, cost-effective)
    if model is None:
...
# default_section captures content before first section marker as "PARALLEL_PROCESSOR"
# _OPT_PARALLEL_PROMPTS = load_prompt_sections("solution_deep_agent_prompts", default_section="INSTRUMENT_IDENTIFICATION")
USER_SPECS_PROMPT = SOLUTION_DEEP_AGENT_PROMPTS["USER_SPECS"]
...
LLM_SPECS_PROMPT = SOLUTION_DEEP_AGENT_PROMPTS["LLM_SPECS"]

# FIX: Removed hard limit - ALL specs should be included in sample_input
# User requirement: Include ALL technical specifications regardless of count
MAX_SPECS_PER_ITEM = 9999  # Effectively unlimited
...
def generate_llm_specs_with_shared_llm(
    ...
    llm: Optional[Any] = None,
    min_specs: int = 60  # Raised to 60 for 60+ KV target
) -> Dict[str, Any]:
    """
    Generate LLM specs using shared LLM instance with iterative enrichment.

    Iterates up to max_iterations times until min_specs unique valid
    specifications are collected.
    ...
    Args:
        ...
        llm: Optional LLM instance (uses shared if None)
        min_specs: Minimum number of specs to generate (default 60)
    ...
    all_specs = {}
    iteration = 0
    max_iterations = 4  # Raised to 4 for 60+ KV target (single call unreliable at 60)
    ...
    try:
        ...
        # =====================================================================
        # CRITICAL: Check MAX_SPECS_PER_ITEM inside loop, not in condition
        # This prevents items from being returned with <min_specs due to iteration exhaustion
        while len(all_specs) < min_specs and iteration < max_iterations:
            # Hard limit check (inside loop to prevent early termination)
            if len(all_specs) >= MAX_SPECS_PER_ITEM:
                logger.info(f"[LLM_SPECS_ITER] Reached hard limit of {MAX_SPECS_PER_ITEM} specs, stopping iterations for {product_type}")
                break
            ...
            try:
                ...
                if json_match:
                    ...
                    for key, value in clean_iter_specs.items():
                        ...
                        if key not in all_specs and is_valid_spec_key(key):
                            all_specs[key] = value
                            new_count += 1
...
def _clean_and_flatten_specs(raw_specs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Handles nested structures and filters out N/A values and invalid keys using shared validation logic.
    """
    clean_specs = {}
    ...
    for k, v in clean_specs.items():
        # FIX: Validate key to prevent hallucinations using shared validator
...
def process_single_product(
    ...
    # Generate LLM specs (minimum 30 — the other 30 come from standards documents)
...
def run_optimized_parallel_enrichment(
    ...
    # ==========================================================================
    # PHASE 2 & 3: Run LLM Specs and Standards Extraction in PARALLEL
    # ==========================================================================

    parallel_start = time.time()

    def run_llm_specs_phase():
        """Phase 2: Process all products for user specs + LLM specs"""
        phase2_start = time.time()
        user_specs_results = {}
        llm_specs_results = {}

        logger.info(f"[OPT_PARALLEL] Phase 2: Processing {len(items)} products (max {max_parallel_products} concurrent)...")
        ...
        return user_specs_results, llm_specs_results

    def run_standards_phase():
        ...
        if not standards_detected:
            ...
            return standards_specs_results, combined_specs_from_step5
    ...
    # Launch both phases in parallel using ThreadPoolExecutor
    logger.info("[OPT_PARALLEL] 🚀 Launching Phase 2 (LLM) and Phase 3 (Standards) in PARALLEL...")

    from concurrent.futures import ThreadPoolExecutor, as_completed

    with ThreadPoolExecutor(max_workers=2) as phase_executor:
        llm_future = phase_executor.submit(run_llm_specs_phase)
        standards_future = phase_executor.submit(run_standards_phase)

        # Wait for both to complete
        user_specs_results, llm_specs_results = llm_future.result()
        standards_specs_results, combined_specs_from_step5 = standards_future.result()

    parallel_time = time.time() - parallel_start
    logger.info(f"[OPT_PARALLEL] ✅ Phase 2 & 3 completed in PARALLEL in {parallel_time:.2f}s (max of both, not sum)")
    logger.info(f"[OPT_PARALLEL] 💡 Time saved vs sequential: ~15-20 seconds")
    ...
    for item in items:
        ...
        # FIX #2: Always compute manual merge, only use Step5 if it has MORE specs
        # Lazy import to avoid circular dependency
        from ..specifications.aggregator import SpecificationAggregator
        manual_merged = SpecificationAggregator.merge_spec_sources(
            user_specs=user_specs,
            standards_specs=standards_specs,
            llm_specs=llm_specs,
            max_specs=MAX_SPECS_PER_ITEM
        )

        # Use Step5 combined_specs ONLY if they have more specs than manual merge
        step5_specs = combined_specs_from_step5.get(item_name, {})
        if step5_specs and len(step5_specs) >= len(manual_merged):
            merged_specs = step5_specs
            logger.info(f"[OPT_PARALLEL-PHASE4] Using Step5 combined_specs for '{item_name}' ({len(merged_specs)} specs)")
        else:
            merged_specs = manual_merged
            logger.info(f"[OPT_PARALLEL-PHASE4] Using manual merge for '{item_name}' ({len(merged_specs)} specs, step5 had {len(step5_specs)})")
        
        # ---------------------------------------------------------------
        # ENFORCE 30+30 SPLIT: 30 from LLM + 30 from Standards = 60
        # ---------------------------------------------------------------
        MIN_PER_SOURCE = 30
        if standards_detected:
            if len(llm_specs) < MIN_PER_SOURCE:
                logger.warning(
                    f"[OPT_PARALLEL] Item '{item_name}': LLM specs below minimum "
                    f"({len(llm_specs)}/{MIN_PER_SOURCE})"
                )
            if len(standards_specs) < MIN_PER_SOURCE:
                logger.warning(
                    f"[OPT_PARALLEL] Item '{item_name}': Standards specs below minimum "
                    f"({len(standards_specs)}/{MIN_PER_SOURCE}) — "
                    f"RAG enrichment will supplement"
                )
        else:
            # No standards — LLM must carry the full total minimum
            if len(llm_specs) < MIN_TOTAL_SPECS_COUNT:
                logger.warning(
                    f"[OPT_PARALLEL] Item '{item_name}': LLM specs below {MIN_TOTAL_SPECS_COUNT} minimum "
                    f"(no standards) ({len(llm_specs)}/{MIN_TOTAL_SPECS_COUNT})"
                )
        # ---------------------------------------------------------------
        ...
        # Now: verify total_specs_count >= MIN_TOTAL_SPECS_COUNT (60) before marking complete
        total_specs = len(merged_specs)
        if total_specs >= MIN_TOTAL_SPECS_COUNT:
            enriched_item["enrichment_source"] = "phase3_optimized"
            logger.info(
                f"[OPT_PARALLEL] Item '{item.get('name', 'Unknown')}' reached minimum specs "
                f"({total_specs}/{MIN_TOTAL_SPECS_COUNT})"
            )
        else:
            ...
            logger.warning(
                ...
                f"(minimum: {MIN_TOTAL_SPECS_COUNT}) - will need RAG enrichment"
            )
        ...
        # === GENERATE SAMPLE_INPUT FROM ENRICHED SPECS ===
        # Build sample_input from combined_specifications so downstream product search
        # has ALL enriched specs, not just the initial ~5 from LLM identification.
        try:
            from common.agentic.workflows.specification_utils import build_sample_input
            enriched_item["sample_input"] = build_sample_input(
                enriched_item,
                project_name=session_id or "Project"
            )
            logger.info(
                f"[OPT_PARALLEL] Generated sample_input for '{item_name}' "
                f"({len(merged_specs)} specs included)"
            )
        except Exception as si_err:
            logger.warning(f"[OPT_PARALLEL] Failed to generate sample_input for '{item_name}': {si_err}")
            # Keep existing sample_input if generation fails
        # === END GENERATE SAMPLE_INPUT ===
    ...
    logger.info(f"[OPT_PARALLEL] Phase 2+3 (Parallel):   {parallel_time:.2f}s")
    ...
    return {
        ...
        "metadata": {
            ...
            "phase_times": {
                ...
                "parallel_s": round(parallel_time, 2),
