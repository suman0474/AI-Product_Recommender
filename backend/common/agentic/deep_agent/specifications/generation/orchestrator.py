...
# Import from canonical location (standards_deep_agent.py, not standards/deep_agent.py)
from common.standards.generation.deep_agent import (
...
def combiner_agent(state: OrchestratorState) -> Dict[str, Any]:
    ...
    - ROOT CAUSE FIX: Validates values before merging
    """
    ...
    for worker_result in state.get("worker_results", []):
        ...
        for key, value in specs.items():
            ...
            # ROOT CAUSE FIX: Validate before merging
            if not _is_valid_spec_value(value):
                filtered_count += 1
                logger.debug(f"[COMBINER] Filtered invalid spec '{key}': {str(value)[:50]}...")
                continue
        ...
        # Track source breakdown (only count valid specs)
        valid_count = sum(1 for k, v in specs.items() if _is_valid_spec_value(v) and k not in all_specs)
        if source not in sources_breakdown:
            sources_breakdown[source] = 0
        sources_breakdown[source] += valid_count

    if filtered_count > 0:
        logger.info(f"[COMBINER] Filtered {filtered_count} invalid/error values")
