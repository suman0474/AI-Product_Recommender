...
class SchemaGenerationDeepAgent:
    ...
    def llm(self):
        ...
        if self._llm is None:
            from common.services.llm.fallback import create_fallback_llm
    ...
    def _run_parallel_sources(self,
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            futures[executor.submit(
                self._fetch_llm_specs, product_type, user_specs
    ...
    def _fetch_llm_specs(self, product_type: str, user_specs: Optional[Dict] = None) -> SourceResult:
        """Fetch specifications from LLM generation."""
        start_time = time.time()

        try:
            from .specifications.generation.llm_generator import generate_llm_specs

            # Build rich context from user specs
            context = "Generated by SchemaGenerationDeepAgent"
            if user_specs:
                spec_str = ", ".join([f"{k}: {v}" for k, v in user_specs.items() if v])
                context = f"Sample Input Context: {spec_str}"
            ...
            # Check for explicit success OR if we got specs back (robustness)
            is_successful = result.get("success", False) or len(specs) > 0
    ...
    def _fetch_template_specs(self, product_type: str) -> SourceResult:
        ...
        try:
            from common.agentic.deep_agent.schema.generation.augmented_product_templates import AugmentedProductTemplates
    ...
    def _check_cache(self, product_type: str) -> Optional[SchemaGenerationResult]:
        ...
        try:
            from common.infrastructure.caching import get_schema_cache
