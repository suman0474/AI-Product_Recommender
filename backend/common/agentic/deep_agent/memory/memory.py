...
class DeepAgentEnrichmentPlan(TypedDict):
    """Enrichment execution plan created by Planner (thread scheduling for deep agent)"""
    main_threads: Dict[str, Dict[str, Any]]
    total_threads: int
    created_at: str


# Backward-compatible alias — use DeepAgentEnrichmentPlan in new code
...
class DeepAgentMemory:
    ...
    def store_taxonomy(self, taxonomy: Dict[str, Any]) -> None:
        """Store product taxonomy"""
        with self._lock:
            self.taxonomy = taxonomy
            logger.info("[Memory] Stored taxonomy")
...
__all__ = [
    ...
    "ExecutionPlan",  # backward-compat alias for DeepAgentEnrichmentPlan
    "SpecificationSource",
