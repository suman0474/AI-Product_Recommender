...
if len(GOOGLE_API_KEYS) > 1:
    ...
    if issue_debug:
        issue_debug._log("SECURITY", f"INIT: Loaded {len(GOOGLE_API_KEYS)} keys for rotation")
...
@debug_log("LLM", log_args=False)
@timed_execution("LLM", threshold_ms=5000)
...
def create_llm_langchain(
    ...
    **kwargs
    ...
    """
    .. deprecated::
        Use `create_llm_with_fallback` directly — it is the canonical function.
        This alias exists for backward compatibility only.
    """
    import warnings
    warnings.warn(
        "create_llm_langchain is deprecated; use create_llm_with_fallback instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
class FallbackLLMClient:
    ...
    Note: Imports from common.infrastructure.state.context.managers are done lazily in methods
    ...
    def __enter__(self):
        ...
        from common.infrastructure.state.context.managers import LLMResourceManager
