...
from common.core.sas_utils import generate_sas_url
...
class ImageService:
    ...
    def cache_generic_image_metadata(
        self,
        product_type: str,
        image_url: str,
        ...
        """
        Cache metadata for a generic image that is ALREADY stored in Azure Blob.
        This is used by image_utils.py which handles the Azure upload itself.

        Args:
            product_type: Product type
            image_url: The SAS URL or public URL of the image
            source: Source of the image (e.g. 'gemini_imagen')
            metadata: Additional metadata (azure_blob_path, content_type, etc.)

        Returns:
            Result dict with success status and cache_id
        """
        if self.generic_images_collection is None:
        ...
        # Get blob path from metadata or default to standard format
        blob_name = metadata.get('azure_blob_path', '')
        if not blob_name:
            # Fallback if metadata missing (unlikely given image_utils usage)
            blob_name = f"generic_images/{normalized}.png"

        # Construct cache document (matching schema of cache_generic_image)
        cache_doc = {
            "id": f"generic_image_{normalized}",
            "type": "generic_product_image",
            "product_type": product_type,
            ...
            "image": {
                "storage": "azure_blob",
                "container": "generic-images", # Hardcoded to match config
                "blobName": blob_name,
                "blobUrl": image_url.split('?')[0]  # Store base URL without SAS token
            },
            "contentType": metadata.get('content_type', 'image/png'),
        ...
        try:
            ...
            return {
                'success': True,
                'cache_id': cache_doc['id'],
                'modified': result.modified_count > 0,
                'upserted': result.upserted_id is not None
            }
        except Exception as e:
            print(f"Generic image metadata cache error: {e}")
