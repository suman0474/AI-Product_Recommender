...
from common.utils.streaming import ProgressEmitter, ProgressStep, with_streaming

# Import all workflow functions
# Import from solution (aliased for backward compatibility)
import importlib as _importlib
_solution_workflow = _importlib.import_module("solution.workflow")
run_solution_workflow = _solution_workflow.run_solution_deep_agent
run_solution_workflow_stream = _solution_workflow.run_solution_deep_agent_stream
from ..base.workflow import run_workflow as _run_workflow
...
def run_instrument_identifier_workflow_stream(
    ...
    """
    # Execute workflow
    result = _run_workflow(user_input, session_id, workflow_type="instrument_identification")
    return result
...
@with_streaming(
    progress_steps=[
        ...
        ProgressStep('schema_retrieval', 'Retrieving product schema from indexed data...', 85),
...
def run_potential_product_indexing_workflow_stream(
    product_type: str,
    session_id: str = "default",
