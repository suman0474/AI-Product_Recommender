...
from common.tools.ranking_tools import rank_products_tool
...
@debug_langgraph_node("WORKFLOW", "classify_intent")
@timed_execution("WORKFLOW", threshold_ms=2000)
def classify_intent_node(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        result = classify_intent_tool.invoke({
            ...
            "current_step": current_step.value if hasattr(current_step, 'value') else (current_step or "start"),
            "context": None
        })
        ...
        state["messages"] = state.get("messages", []) + [{
            "role": "system",
            "content": f"Intent classified as: {state['intent'].value if hasattr(state['intent'], 'value') else state['intent']} (confidence: {state['intent_confidence']:.2f})"
        }]

        _iv = lambda x: x.value if hasattr(x, 'value') else x
        logger.info(f"[WORKFLOW] Intent: {_iv(state['intent'])}, Next: {_iv(state['next_step'])}")
...
@debug_langgraph_node("WORKFLOW", "validate_requirements")
...
def create_instrument_identification_workflow() -> StateGraph:
    ...
    _id_agents = _il.import_module("solution.identification_agents")

    def identify_instruments_node(state: WorkflowState) -> WorkflowState:
        """Identify instruments from requirements"""
        result = _id_agents.InstrumentIdentificationAgent().identify(
    ...
    def identify_accessories_node(state: WorkflowState) -> WorkflowState:
        ...
        if state.get("instruments"):
            result = _id_agents.AccessoryIdentificationAgent().identify(
...
def run_workflow(
    ...
    try:
        ...
        return {
            ...
            "intent": final_state.get("intent").value if hasattr(final_state.get("intent"), 'value') else final_state.get("intent"),
            "product_type": final_state.get("product_type"),
            "requires_user_input": final_state.get("requires_user_input", False),
            "current_step": final_state.get("current_step").value if hasattr(final_state.get("current_step"), 'value') else final_state.get("current_step"),
