...
from common.tools.ranking_tools import judge_analysis_tool, rank_products_tool
# DEPRECATED: search_product_images_tool, search_pdf_datasheets_tool dissolved into search
search_product_images_tool = None
search_pdf_datasheets_tool = None
...
def identify_instruments_and_accessories_node(
    ...
    try:
        ...
        agents = _get_id_agents()
        inst_result = agents.InstrumentIdentificationAgent().identify(
        ...
        if not state.get("identified_accessories"):
            acc_result = agents.AccessoryIdentificationAgent().identify(
...
def parallel_vendor_analysis_node(
    ...
    try:
        ...
        from common.infrastructure.state.execution.executor_extensions import ThreadSafeResultCollector
