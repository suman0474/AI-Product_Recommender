...
class BaseRAGAgent(ABC):
    ...
    def _create_prompt(self) -> ChatPromptTemplate:
        ...
        if self.PROMPT_KEY:
            # Map section names to common.prompts constants
            from common import prompts as common_prompts
            
            section_map = {
                "rag_prompts": getattr(common_prompts, "RAG_PROMPTS", {}),
                "index_rag_prompts": getattr(common_prompts, "INDEX_RAG_PROMPTS", {}),
                "solution_deep_agent_prompts": getattr(common_prompts, "SOLUTION_DEEP_AGENT_PROMPTS", {}),
                "search_deep_agent_prompts": getattr(common_prompts, "SEARCH_DEEP_AGENT_PROMPTS", {}),
                "standards_deep_agent_prompts": getattr(common_prompts, "STANDARDS_DEEP_AGENT_PROMPTS", {}),
            }
            
            if self.PROMPT_SECTION in section_map:
                prompts = section_map[self.PROMPT_SECTION]
            else:
                 # Fallback: try to lookup attribute directly if it matches upper case
                 upper_section = self.PROMPT_SECTION.upper()
                 if hasattr(common_prompts, upper_section):
                     prompts = getattr(common_prompts, upper_section)
                 else:
                     raise ValueError(f"Prompt section '{self.PROMPT_SECTION}' not found in common.prompts mappings")
