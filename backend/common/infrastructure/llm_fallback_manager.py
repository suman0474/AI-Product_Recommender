...
from common.infrastructure.circuit_breaker import get_circuit_breaker, CircuitBreakerError
...
def get_llm_fallback_manager(enable_cache: bool = False) -> LLMFallbackManager:
    """
    Get or create global LLM fallback manager (multi-provider: Gemini → GPT-4 → GPT-3.5).

    Use this when you need multi-provider fallback with circuit breakers.
    For single-provider (Gemini) with quota tracking, use `common.utils.llm_manager.get_llm_manager`.
...
# Backward-compatible alias — use get_llm_fallback_manager in new code
get_llm_manager = get_llm_fallback_manager
