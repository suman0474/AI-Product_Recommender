...
def product_search():
    ...
    try:
        ...
        # Extract thread/session identifiers
        import uuid as _uuid
        main_thread_id = data.get('main_thread_id') or f"main_legacy_{_uuid.uuid4().hex[:12]}"
        workflow_thread_id = data.get('workflow_thread_id') or data.get('thread_id') or f"product_search_legacy_{_uuid.uuid4().hex[:12]}"
        zone = data.get('zone', 'DEFAULT')
        session_id = data.get('session_id') or data.get('search_session_id') or workflow_thread_id
