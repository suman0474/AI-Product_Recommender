...
from .workflow import get_strategy_for_product
...
def enrich_with_strategy_rag(
    ...
    Returns:
        ...
        - preferred_vendors: List of preferred vendors
        - forbidden_vendors: Empty list
        ...
        - sources_used: ["mongodb"]
    """
    try:
        logger.info(f"[Strategy Enrichment] Getting MongoDB strategy for {product_type}")
        ...
        if result.get('preferred_vendors') or result.get('neutral_vendors'):
            logger.info(f"[Strategy Enrichment] MongoDB strategy success - "
            ...
            logger.warning("[Strategy Enrichment] No vendors found in MongoDB for this category")
            return {
                'success': False,
                'rag_type': 'mongodb',
...
def filter_vendors_by_strategy_data(
    ...
    """
    preferred = []
    raw_preferred = strategy_data.get('preferred_vendors', [])
    if isinstance(raw_preferred, list):
        for v in raw_preferred:
            if isinstance(v, str):
                preferred.append(v.lower())
            elif isinstance(v, list) and v and isinstance(v[0], str):
                # Handle nested list case e.g. [["Vendor A"]]
                preferred.append(v[0].lower())
            else:
                # Fallback to string conversion
                preferred.append(str(v).lower())
    ...
    for vendor in vendors:
        if not isinstance(vendor, str):
            logger.warning(f"[StrategyEnrichment] Skipping invalid vendor type: {type(vendor)}")
            continue

        vendor_lower = vendor.lower()
        is_preferred = any(p in vendor_lower or vendor_lower in p for p in preferred)

        # Rule-based priority scoring from MongoDB strategy data
        priority_score = 0
        for pv, score in priorities.items():
            if isinstance(pv, str) and (pv.lower() in vendor_lower or vendor_lower in pv.lower()):
...
def enrich_with_strategy_llm_fallback(
    ...
    Now delegates to MongoDB-based enrichment.
    """
    logger.warning("[Strategy] enrich_with_strategy_llm_fallback is deprecated. "
                   "Using MongoDB-based strategy instead.")
    return enrich_with_strategy_rag(
        product_type=product_type,
        requirements=requirements
    )
