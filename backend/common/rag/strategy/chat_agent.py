...
STRATEGY_CHAT_PROMPT = RAG_PROMPTS["STRATEGY_CHAT"]
...
class StrategyChatAgent:
    """
    Specialized agent for answering strategy questions using MongoDB vendor strategy data.

    Retrieves vendor strategy records from MongoDB, constructs context,
    ...
    def __init__(self, llm=None, temperature: float = 0.1):
        ...
        logger.info("StrategyChatAgent initialized (MongoDB-backed)")

    def retrieve_documents(self, question: str, product_type: str = "", top_k: int = 7, user_id: int = None) -> Dict[str, Any]:
        """
        Retrieve relevant strategy documents from MongoDB.
        ...
        """
        try:
            # Load all vendor records from MongoDB
            mongo_result = load_strategy_from_mongodb(user_id=user_id)
            all_vendors = mongo_result.get("vendors", [])

            if not all_vendors:
                return {
                    "results": [],
                    "result_count": 0,
                    "error": mongo_result.get("error", "No strategy documents found")
                }

            # Filter by product_type if provided (simple keyword match)
            search_term = (product_type or question or "").lower()
            if search_term:
                scored = []
                for v in all_vendors:
                    ...
                    score = sum(1 for word in search_term.split() if word in text)
                    scored.append((score, v))
                scored.sort(key=lambda x: x[0], reverse=True)
                results = [v for _, v in scored[:top_k] if _[0] > 0] or [v for _, v in scored[:top_k]]
            else:
                results = all_vendors[:top_k]

            # Normalise to expected format
            formatted = []
            for rec in results:
                formatted.append({
                    "content": f"{rec.get('category', '')} | {rec.get('subcategory', '')} | "
                               f"{rec.get('vendor_name', '')}: {rec.get('strategy', '')}",
                    "metadata": {
                        "filename": "mongodb:stratergy",
                        "document_type": "strategy",
                        "vendor_references": [rec.get("vendor_name", "")],
                        "policy_references": [],
                    },
            ...
            logger.info(f"[StrategyChatAgent] Retrieved {len(formatted)} strategy records from MongoDB")
            return {"results": formatted, "result_count": len(formatted)}

        except Exception as e:
            logger.error(f"[StrategyChatAgent] Error retrieving strategy documents: {e}")
