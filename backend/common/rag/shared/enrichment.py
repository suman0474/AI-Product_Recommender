...
# - Solution Deep Agent (user-centric specification extraction)
#
...
class UnifiedEnrichmentEngine:
    ...
    def _extract_standards_specs(self, standards_context: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            from common.rag.standards.enrichment import StandardsEnrichment
    ...
    def _extract_vendor_specs(self, vendor_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            from common.services.extraction.extraction_engine import aggregate_results
...
def integrate_with_ppi_workflow():
    ...
    In Indexing.deep_agent_ppi workflow (schema_architect agent):

    from common.rag.shared.enrichment import enrich_product_with_60_plus_specs
...
def integrate_with_solution_workflow():
    """
    Integration points for Solution Deep Agent:

    In solution/workflow.py parallel_enrichment_node():
