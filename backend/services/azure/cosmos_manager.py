...
class CosmosProjectManager:
    ...
    def save_project(self, user_id: str, project_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ensure critical fields have defaults (for backward compatibility with old projects)
            project_data.setdefault('session_context', {})
            project_data.setdefault('workflow_state', {})
            project_data.setdefault('intent_data', {})

            # Debug logging for critical fields
            self.logger.info(f"Saving project {project_id} - Field presence check:")
            self.logger.info(f"  session_context: {bool(project_data.get('session_context'))}")
            self.logger.info(f"  workflow_state: {bool(project_data.get('workflow_state'))}")
            self.logger.info(f"  intent_data: {bool(project_data.get('intent_data'))}")
            ...
            complete_project_data = {
                ...
                'session_context': project_data.get('session_context', {}),  # Session continuity
                'intent_data': project_data.get('intent_data', {}),  # Workflow intent tracking
    ...
    def get_project_details(self, project_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ensure backward compatibility: Add missing fields for old projects
            if 'session_context' not in project_data or not project_data['session_context']:
                project_data['session_context'] = {}
                self.logger.info(f"Added default session_context for old project: {project_id}")

            if 'workflow_state' not in project_data or not project_data['workflow_state']:
                project_data['workflow_state'] = {}
                self.logger.info(f"Added default workflow_state for old project: {project_id}")

            if 'intent_data' not in project_data or not project_data['intent_data']:
                project_data['intent_data'] = {}
                self.logger.info(f"Added default intent_data for old project: {project_id}")
