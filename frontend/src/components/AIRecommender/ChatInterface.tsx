    ...
    >
      ...
      >
        ...
        <div className="flex-1">
          ...
          >
            <div>
              {/* Separate content logic */}
              {(() => {
                // 1. Process content to extract solution/chat links
                let content = message.content.replace(/<a\s+href="([^"]+)"[^>]*>(.*?)<\/a>/g, '[$2]($1)');
                const links: { href: string; text: string; isSolution: boolean }[] = [];

                // Regex to find markdown links: [text](href)
                const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
                let match;

                // Temporary placeholder to avoid modifying string while iterating? 
                // Better: First pass to find, second to replace or just render buttons separately?
                // Actually, let's just find them and remove them from display content if we want them ONLY at bottom

                // Strategy:
                // 1. Find relevant links
                // 2. Remove them from the text content to be rendered by ReactMarkdown
                // 3. Render them as buttons below

                const buttons: JSX.Element[] = [];

                // Custom replace to extract and remove specific links
                const cleanContent = content.replace(linkRegex, (fullMatch, text, href) => {
                  const isSolution = href.includes('/solution') || text.includes('Open Solution Page');
                  const isChat = href.includes('/chat') || text.includes('Open Chat Page');

                  if (isSolution || isChat) {
                    buttons.push(
                      <a
                        key={href}
                        href={href}
                        ...
                        className="px-4 py-2.5 rounded-lg font-semibold text-white text-sm transition-all duration-300 hover:scale-105 hover:shadow-lg active:scale-95 flex items-center gap-2 inline-flex no-underline mt-4 mb-1"
                        style={{
                          background: '#0F6CBD',
                          boxShadow: '0 4px 15px rgba(15, 108, 189, 0.4)'
                        }}
                      >
                        {/* Add icons based on type if needed, strict match to image requested */}
                        {isSolution && <span className="mr-1">🔧 📋</span>}
                        {isChat && <span className="mr-1">💬</span>}
                        {text}
                      </a>
                    );
                    return ''; // Remove from text
                  }
                  return fullMatch; // Keep other links
                });
                ...
                return (
                  <>
                    ...
                    {/* Render extracted buttons at the bottom */}
                    {buttons.length > 0 && (
...
const ChatInterface = ({
  ...
  const getPlaceholderText = () => {
    ...
    switch (currentStep) {
      case "greeting":
        return "What product are you looking for? (e.g., Pressure Transmitter, RTD, Thermocouple)...";
      case "initialInput":
        return "Describe the product you are looking for...";
      case "awaitMissingInfo":
        return "Provide the missing specifications (e.g., range, material, output)...";
      case "awaitAdditionalAndLatestSpecs":
        return "Add any additional or latest specifications...";
      case "awaitAdvancedSpecs":
        return "Specify advanced technical parameters...";
      case "confirmAfterMissingInfo":
        return "Type 'Yes' to confirm or provide corrections...";
      case "showSummary":
